...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-miningchainguard", "Report local mining chain-alignment health from outbound peers; peer-derived health is advisory and only local bootstrap states pause template creation (default: 1 on mainnet, 0 on test chains)", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningchainguardminpeers=<n>", strprintf("Minimum outbound peers with usable tip heights expected by the mining chain-alignment advisory (default: %d)", node::DEFAULT_MINING_CHAIN_GUARD_MIN_PEERS), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningchainguardminneartippeers=<n>", strprintf("Minimum outbound peers that should be within -miningchainguardneartipwindow blocks of the local tip for a healthy advisory status (default: %d)", node::DEFAULT_MINING_CHAIN_GUARD_MIN_NEAR_TIP_PEERS), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningchainguardmaxmediangap=<n>", strprintf("Maximum advisory block gap between the local tip and the median outbound peer tip before chain_guard reports continue_with_warning (default: %d)", node::DEFAULT_MINING_CHAIN_GUARD_MAX_MEDIAN_GAP), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningchainguardneartipwindow=<n>", strprintf("Block window used to count near-tip outbound peers for local mining chain-alignment advisory status (default: %d)", node::DEFAULT_MINING_CHAIN_GUARD_NEAR_TIP_WINDOW), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningchainguardstalepeerseconds=<n>", strprintf("Seconds after which lagging peers without fresh block signals are ignored by local mining chain-alignment advisory status (default: %d)", node::DEFAULT_MINING_CHAIN_GUARD_STALE_PEER_SECONDS), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
