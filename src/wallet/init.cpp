...
#endif
    argsman.AddArg("-autoshieldcoinbase", "Automatically shield mature coinbase outputs into the shielded pool when a new block arrives (default: false). Opt-in for fund preservation: mined rewards stay as post-quantum transparent (P2MR, ML-DSA/SLH-DSA) outputs, which carry no shielded-proof soundness exposure, until the operator chooses privacy. Even when enabled, auto-shielding does not begin until the C-002 shielded-pool hardening activates (block 123,000 on mainnet; see -autoshieldcoinbaseminheight).", ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    argsman.AddArg("-autoshieldcoinbaseminheight=<n>", "Do not auto-shield coinbase below this block height (default: the C-002 shielded-pool hardening height, 123000 on mainnet). Lower it (e.g. 0) only on test networks where C-002 is effectively active early.", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::WALLET);
